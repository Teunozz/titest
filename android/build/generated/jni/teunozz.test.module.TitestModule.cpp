...
Persistent<FunctionTemplate> TitestModule::proxyTemplate = Persistent<FunctionTemplate>();
jclass TitestModule::javaClass = NULL;

TitestModule::TitestModule(jobject javaObject) : titanium::Proxy(javaObject)
{
}

void TitestModule::bindProxy(Handle<Object> exports)
{
...
// Methods --------------------------------------------------------------------
Handle<Value> TitestModule::start(const Arguments& args)
{
	LOGD(TAG, "start()");
	HandleScope scope;
	...
	if (!methodID) {
		methodID = env->GetMethodID(TitestModule::javaClass, "start", "()V");
		if (!methodID) {
...
Handle<Value> TitestModule::sendData(const Arguments& args)
{
	LOGD(TAG, "sendData()");
	HandleScope scope;
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(TitestModule::javaClass, "sendData", "()V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'sendData' with signature '()V'";
			LOGE(TAG, error);
				return titanium::JSException::Error(error);
		}
	}
...
Handle<Value> TitestModule::stop(const Arguments& args)
{
	LOGD(TAG, "stop()");
	HandleScope scope;
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(TitestModule::javaClass, "stop", "()V");
		if (!methodID) {
